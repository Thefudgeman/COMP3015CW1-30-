...
to add fog to the scene. There are seperate shaders for texture mixing as that will stop any issues with using mutiple textures in one fragment shader when I only want to use one. There is also a seperate shader for the skybox.
...
\+ To increase the fog and - to decrease it (when pressing + after fog completly hides scene it will appear to be removed. Press - to make it return) <br>
...
Assiting with borken normal mapping (did not work) <br>
